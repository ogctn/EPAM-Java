...
class NewPostOfficeTest {
    Logger log = LoggerFactory.getLogger(NewPostOfficeTest.class);
    ...
    void testAddDeclineBox(Collection<Box> boxes, Collection<Integer> values, Collection<BigDecimal> expected, Collection<BigDecimal> declinedExpected, int percent) {
        ...
        List<BigDecimal> actual = office.getListBox().stream()
                .map(Box::getCost)
                .collect(Collectors.toList());
        Iterator<BigDecimal> expectedIt = expected.iterator();
        actual.forEach(e -> {
            BigDecimal act = e.setScale(12, RoundingMode.HALF_EVEN);
            BigDecimal exp = expectedIt.next().setScale(12, RoundingMode.HALF_EVEN);
            log.debug("exp -> {},  act -> {}", exp, act);
            assertEquals(exp, act, "exp -> " + exp + ", act -> " + act);
        });

        office.declineCostOfBox(percent);
        Iterator<BigDecimal> actualIt = office.getListBox().stream().map(Box::getCost).iterator();
        declinedExpected.forEach(e -> {
            BigDecimal exp = e.setScale(12, RoundingMode.HALF_EVEN);
            BigDecimal act = actualIt.next().setScale(12, RoundingMode.HALF_EVEN);
            log.debug("exp -> {},  act -> {}", exp, act);
            assertEquals(exp, act, "exp -> " + exp + ", act -> " + act);
        });
    }
    ...
    void testCalculateCostOfBox(double weight, double volume, int value, BigDecimal expected) {
        assertEquals(expected.setScale(12, RoundingMode.HALF_EVEN),
                NewPostOffice.calculateCostOfBox(weight, volume, value).setScale(12, RoundingMode.HALF_EVEN));
    }
